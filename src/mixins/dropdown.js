...
const Selector = {
  ...
  ITEM_SELECTOR: [".dropdown-item", ".nly-dropdown-form"]
...
export default {
  ...
  provide() {
    return { nlyaDropdown: this };
  },
  inject: {
    nlyaNavbar: { default: null }
  },
  props: {
    disabled: {
      type: Boolean,
  ...
  watch: {
    visible(newValue, oldValue) {
      ...
      if (newValue !== oldValue) {
        ...
        const nlyEvt = new NlyEvent(evtName, {
          cancelable: true,
          vueTarget: this,
  ...
  methods: {
    ...
    emitEvent(nlyEvt) {
      ...
      this.$root.$emit(`${ROOT_DROPDOWN_PREFIX}${type}`, nlyEvt);
    },
    showMenu() {
      if (this.disabled) {
        /* istanbul ignore next */
    ...
    toggle(evt) {
      ...
      if (
        ...
        !(
          type === "keydown" &&
          [KeyCodes.ENTER, KeyCodes.SPACE, KeyCodes.DOWN].indexOf(keyCode) !==
            -1
        )
      ) {
        /* istanbul ignore next */
