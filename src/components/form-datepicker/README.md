    ...
    data() {
      ...
      return {
        ...
        validOptinos: [
          ...
          {
            value: "invalid",
            text: "显示invalid feedback"
          },
          {
            value: "valid",
            text: "显示 valid feedback"
          },
          {
            value: "warning",
            text: "显示 warning feedback"
          }
        ]
      };
    },
...
`value` 是 日期选择器选择的时间，给 `value` 传值可以初始化日期选择器的初始值。
...
使用 `locale-data` 可以格式化 input 框中时间显示的格式，比如设置 `local-data = { firstDay: 1, format: 'yyyy-mm-dd HH:MM:ss' }`
...
除了使用 `v-model = 'value'` 可以双向绑定数据之外，可以使用 `update` 方法来获取日期选择器选择的时间

```html
<template>
  <div>
    <nly-form-datepicker :value="dateRange" @update="update" />
    <nly-form-group label="起始时间">
      <nly-form-input v-model="dateRange.startDate" type="text" />
    </nly-form-group>
    <nly-form-group label="结束时间">
...
- 默认渲染下， 不支持从日期选择器中输入日期来调整日历上的时间 ###
  最小和最大时间限制 设置 `min-date` 和 `max-date` prop 可以限制日期选择器的
  时间选择范围，超出这个范围的时间是无法选择的，且日历上也无法联动现实超出这个范围的时间
