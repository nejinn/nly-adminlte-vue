  ...
  methods: {
    ...
    renderTbodyRowCell(field, colIndex, item, rowIndex) {
      ...
      if (stickyColumn) {
        ...
        data.attrs["data-label"] =
          this.isStacked && !isUndefinedOrNull(field.label)
    ...
    renderTbodyRow(item, rowIndex) {
      ...
      if (this.currentPage && this.perPage && this.perPage > 0) {
        ariaRowIndex = String(
          (this.currentPage - 1) * this.perPage + rowIndex + 1
        );
      }
      ...
      const rowId = primaryKeyValue
        ? this.safeId(`_row_${primaryKeyValue}`)
        : null;

      // Selectable classes and attributes
      const selectableClasses = this.selectableRowClasses
        ? this.selectableRowClasses(rowIndex)
        : {};
      const selectableAttrs = this.selectableRowAttrs
      ...
      $rows.push(
        h(
          NlyTr,
          {
            key: `__nly-table-row-${rowKey}__`,
            ref: "itemRows",
            refInFor: true,
            class: [
              userTrClasses,
              selectableClasses,
              rowShowDetails ? "nly-table-has-details" : ""
            ],
      ...
      if (rowShowDetails) {
        ...
        const $details = h(
          NlyTd,
          { props: { colspan: fields.length }, class: this.detailsTdClass },
        ...
        if (tableStriped) {
          $rows.push(
            ...
            h("tr", {
              key: `__nly-table-details-stripe__${rowKey}`,
              staticClass: "d-none",
        ...
        $rows.push(
          h(
            NlyTr,
            {
              key: `__nly-table-details__${rowKey}`,
