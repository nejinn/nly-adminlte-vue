/**
 * Plugin for adding `$nlyaToast` property to all Vue instances
 */
...
// Method to install `$nlyaToast` VM injection
const plugin = Vue => {
  ...
  // Declare NlyaToast instance property class
  class NlyaToast {
    constructor(vm) {
  ...
  Vue.mixin({
    beforeCreate() {
      ...
      // we have to create a fresh instance of `NlyaToast` for each VM
      this[PROP_NAME_PRIV] = new NlyaToast(this);
    }
  });

  // Define our read-only `$nlyaToast` instance property
...
export const NLYAToastPlugin = pluginFactory({
  plugins: { plugin }
});
