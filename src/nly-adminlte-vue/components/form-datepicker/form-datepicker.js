...
const getConfigFallback = prop => {
  return (
    getComponentConfig(NAME, prop) || getComponentConfig("BCalendar", prop)
  );
};
...
export const BFormDatepicker = /*#__PURE__*/ Vue.extend({
  ...
  methods: {
    ...
    onTodayButton() {
      ...
      this.setAndClose(
        formatYMD(constrainDate(createDate(), this.min, this.max))
      );
    },
    onResetButton() {
    ...
    defaultButtonFn({ isHovered, hasFocus }) {
      return this.$createElement(
        isHovered || hasFocus ? BIconCalendarFill : BIconCalendar,
        {
          props: { scale: 1.25 },
